...
def get_stocks(
    ...
    try:
        with get_sqlite_db() as cursor:
            if query:
                cursor.execute(
                    "SELECT ticker, close_price, volume FROM stock_info WHERE ticker LIKE ? LIMIT ?",
                    (f"%{query}%", limit)
                )
            else:
                cursor.execute(
                    "SELECT ticker, close_price, volume FROM stock_info LIMIT ?",
                    (limit,)
                )
...
def get_sorted_stocks(
    ...
    sort_column = "close_price" if sort_by == "price" else "volume"
    ...
    try:
        with get_sqlite_db() as cursor:
            ...
            query_sql = f"SELECT ticker, close_price, volume FROM stock_info ORDER BY {sort_column} {order.upper()} LIMIT ? OFFSET ?"
