...
def load_api_key():
    """Load VNSTOCK API key from environment variable"""
    # Try to get API key from environment variable
    api_key = os.getenv('VNSTOCK_API')
    
    if api_key:
        print(f"VNSTOCK API key loaded from environment variable")
        return api_key
    else:
        print("VNSTOCK_API not found. Using guest mode (rate limit: 20 req/min)")
        return None
...
def ingest_data():
    ...
    if all_data:
        ...
        # Convert date to string for Spark compatibility
